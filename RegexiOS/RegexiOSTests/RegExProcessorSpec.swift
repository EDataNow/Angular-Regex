            ...
            describe("Return from postion"){
                context("it should should return all characters proceeding from given postion"){
                    it("if match is found and length is not specified"){
                        expect(regex.matchOnPositionReturnFromPosition(input_string:"1", start_index: 2, specific_length: 0, select_index: 5)).to(equal(["45-A500-H1"]))
                    }
                    it("if match is found and length and input_string is not specified"){
                        expect(regex.matchOnPositionReturnFromPosition(input_string:"", start_index: 2, specific_length: 0, select_index: 5)).to(equal(["45-A500-H1"]))
                    }
